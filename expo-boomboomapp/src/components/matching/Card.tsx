...
export function Card({profile, onNext, index, setCurrentIdBackground}: CardProps) {
    ...
    useEffect(() => {
        let cb = onNext.current.subscribe((isYes: boolean) => {
            if (index === 0) {
    ...
    return (
        <Animated.View style={{
            position: 'absolute',
            zIndex: 1000 - index, ...coreStyle.MATCHING_CARD,
            transform: [{scale}, {translateY}, {translateX}, {rotate}],
            backgroundColor
        }}>
            <CardContent profile={profile} setCurrentIdBackground={setCurrentIdBackground}/>
        </Animated.View>
    )
